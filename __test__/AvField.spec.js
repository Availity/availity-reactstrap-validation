...
describe('AvField', function () {
  ...
  describe('structure', () => {
    describe('default', () => {
      ...
      describe('grid', () => {
        beforeEach(() => {
          this.wrapper = shallow(<AvField {...stdProps} grid={{xs: 8}}>Yo!</AvField>, options);
        });
    ...
    describe('with help message', () => {
      describe('default', () => {
        beforeEach(() => {
          this.wrapper = shallow(<AvField {...stdProps} helpMessage="yo">Yo!</AvField>, options);
        });
      ...
      describe('grid', () => {
        beforeEach(() => {
          this.wrapper = shallow(<AvField {...stdProps} grid={{xs: 8}} helpMessage="yo">Yo!</AvField>, options);
        });
  ...
  describe('AvGroup', ()=> {
    it('should pass the "row" prop through', () => {
      const wrapper = shallow(<AvField {...stdProps} grid={{xs: 8}}>Yo!</AvField>, options);
      expect(wrapper.prop('row')).to.be.true;
    });

    it('should pass the "disabled" prop through', () => {
      const wrapper = shallow(<AvField {...stdProps} disabled>Yo!</AvField>, options);
      expect(wrapper.prop('disabled')).to.be.true;
    });

    it('should spread the "groupAttrs" prop on it', () => {
      const wrapper = shallow(<AvField {...stdProps} groupAttrs={{className: 'yo'}}>Yo!</AvField>, options);
  ...
  describe('Label', ()=> {
    it('should pass the "id" prop as the "for" prop', () => {
      const label = shallow(<AvField {...stdProps} id="myId">Yo!</AvField>, options).childAt(0);
    ...
    it('should get the "size" prop', () => {
      const label = shallow(<AvField {...stdProps} size="amazing">Yo!</AvField>, options).childAt(0);
      expect(label.prop('size')).to.equal('amazing');
    });

    it('should use the labelHidden prop as the hidden prop', () => {
      const label = shallow(<AvField {...stdProps} labelHidden>Yo!</AvField>, options).childAt(0);
      expect(label.prop('hidden')).to.be.true;
    });

    it('should spread the "labelAttrs" prop on it', () => {
      const label = shallow(<AvField {...stdProps} labelAttrs={{className: 'yo'}}>Yo!</AvField>, options).childAt(0);
    ...
    it('should have xs prop set when grid is set', () => {
      const label = shallow(<AvField {...stdProps} grid={{xs:8}}>Yo!</AvField>, options).childAt(0);
      expect(label.prop('xs')).to.equal(4);
    });

    it('should have sm prop set when grid is set', () => {
      const label = shallow(<AvField {...stdProps} grid={{sm:8}}>Yo!</AvField>, options).childAt(0);
      expect(label.prop('sm')).to.equal(4);
    });

    it('should have md prop set when grid is set', () => {
      const label = shallow(<AvField {...stdProps} grid={{md:8}}>Yo!</AvField>, options).childAt(0);
      expect(label.prop('md')).to.equal(4);
    });

    it('should have lg prop set when grid is set', () => {
      const label = shallow(<AvField {...stdProps} grid={{lg:8}}>Yo!</AvField>, options).childAt(0);
      expect(label.prop('lg')).to.equal(4);
    });

    it('should have xl prop set when grid is set', () => {
      const label = shallow(<AvField {...stdProps} grid={{xl:8}}>Yo!</AvField>, options).childAt(0);
      expect(label.prop('xl')).to.equal(4);
    });

    it('should have all the col prop set when grid is set', () => {
      const label = shallow(<AvField {...stdProps} grid={{xs:6,md:7,xl:8,other:10}}>Yo!</AvField>, options).childAt(0);
  ...
  describe('AvInput', () =>{
    it('should get other props passed through', () => {
      const input = shallow(<AvField {...stdProps} style={{textAlign: 'center'}}>Yo!</AvField>, options).childAt(1);
    ...
    it('should set "disabled" prop if provided', () => {
      const input = shallow(<AvField {...stdProps} disabled>Yo!</AvField>, options).childAt(1);
      expect(input.prop('disabled')).to.be.true;
    });

    it('should set "readOnly" prop if provided', () => {
      const input = shallow(<AvField {...stdProps} readOnly>Yo!</AvField>, options).childAt(1);
      expect(input.prop('readOnly')).to.be.true;
    });

    it('should set "size" prop if provided', () => {
      const input = shallow(<AvField {...stdProps} size="amazing">Yo!</AvField>, options).childAt(1);
      expect(input.prop('size')).to.equal('amazing');
    });

    it('should have xs prop set when grid is set', () => {
      const input = shallow(<AvField {...stdProps} grid={{xs:8}}>Yo!</AvField>, options).childAt(1);
      expect(input.prop('xs')).to.equal(8);
    });

    it('should have sm prop set when grid is set', () => {
      const input = shallow(<AvField {...stdProps} grid={{sm:8}}>Yo!</AvField>, options).childAt(1);
      expect(input.prop('sm')).to.equal(8);
    });

    it('should have md prop set when grid is set', () => {
      const input = shallow(<AvField {...stdProps} grid={{md:8}}>Yo!</AvField>, options).childAt(1);
      expect(input.prop('md')).to.equal(8);
    });

    it('should have lg prop set when grid is set', () => {
      const input = shallow(<AvField {...stdProps} grid={{lg:8}}>Yo!</AvField>, options).childAt(1);
      expect(input.prop('lg')).to.equal(8);
    });

    it('should have xl prop set when grid is set', () => {
      const input = shallow(<AvField {...stdProps} grid={{xl:8}}>Yo!</AvField>, options).childAt(1);
      expect(input.prop('xl')).to.equal(8);
    });

    it('should have all the col prop set when grid is set', () => {
      const input = shallow(<AvField {...stdProps} grid={{xs:6,md:7,xl:8,other:10}}>Yo!</AvField>, options).childAt(1);
